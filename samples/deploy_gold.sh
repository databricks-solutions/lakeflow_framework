#!/bin/bash
# Gold Sample Bundle Deployment

# Sample-specific constants
BUNDLE_NAME="Gold Sample Bundle"
SCHEMA="${DEFAULT_SCHEMA_NAMESPACE}_gold"
...
if [[ -n "$schema_namespace" ]]; then
    ...
    schema="${schema_namespace}_gold$logical_env"
else
...
if ! update_substitutions_file "gold_sample/src/pipeline_configs/dev_substitutions.json"; then
    log_error "Failed to update substitutions file. Exiting."
    exit 1
fi

# Change to gold_sample directory for deployment
cd gold_sample
...
restore_substitutions_file "gold_sample/src/pipeline_configs/dev_substitutions.json"