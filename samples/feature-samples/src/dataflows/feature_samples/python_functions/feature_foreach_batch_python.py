...
def micro_batch_function(df: DataFrame, batch_id: int, tokens: Dict) -> DataFrame:
    ...
    table_name = f"{bronze_schema}.tgt_foreach_batch_python"
    ...
    df_transformed.write \
        .format("delta") \
        .mode("append") \
        .save(f"{volume_root_file_path}/tgt_foreach_batch_python/delta_target")
    
    # JSON file location
    df_transformed.write \
        .format("json") \
        .mode("append") \
        .save(f"{volume_root_file_path}/tgt_foreach_batch_python/json_target")
