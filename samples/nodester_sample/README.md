...
The Nodester dataflow type (`dataFlowType: "nodester"`) provides compatibility with [node-based](https://github.com/Mmodarre/Lakehouse_Nodester) style specifications. This allows users familiar with Nodester's graph-based approach to define pipelines using source, transformation, and target nodes.
...
│   │   ├── nodester_base_samples_pipeline.yml
│   │   ├── nodester_customer_simple_pipeline.yml
│   │   ├── nodester_customer_transform_pipeline.yml
│   │   ├── nodester_customer_multi_target_pipeline.yml
│   │   └── nodester_customer_cloudfiles_pipeline.yml
...
│   ├── dataflows/
│   │   └── base_samples/
│   │       ├── dataflowspec/         # Nodester-style dataflow specs
...
```bash
# For classic compute
cp resources/classic/nodester_base_samples_pipeline.yml scratch/resources/

# For serverless
cp resources/serverless/nodester_base_samples_pipeline.yml scratch/resources/
```
...
```bash
databricks bundle run lakeflow_samples_nodester_base_pipeline
```
...
### Creating New Nodester Specs

1. Create a new `*_main.json` file in `src/dataflows/base_samples/dataflowspec/`
2. Use `dataFlowType: "nodester"` and define your nodes
...
The Nodester specs are automatically converted to flow specs at runtime, so all framework features (CDC, data quality, secrets, etc.) work seamlessly.
