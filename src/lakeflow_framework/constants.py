...
class FrameworkPaths:
    ...
    CONFIG_PATH and CONFIG_OVERRIDE_PATH are static path segments. At runtime, which root to use
    for framework config files should be chosen using utility.resolve_framework_config_path(framework_path).

    Note: CONFIG_PATH and the schema paths point into the ``lakeflow_framework/`` package subtree
    so that ``importlib.resources`` can locate them when the package is installed as a wheel, while
    flat-deploy customers using ``framework.sourcePath`` continue to resolve them on disk.

    Attributes:
        CONFIG_PATH (str): Path to the config/default directory (inside the lakeflow_framework package tree).
    ...
    """
    CONFIG_PATH: str = "./lakeflow_framework/config/default"
    ...
    # Spec schema definitions paths (inside the lakeflow_framework package tree)
    ...
    TEMPLATE_SPEC_SCHEMA_PATH: str = "./lakeflow_framework/schemas/spec_template.json"
