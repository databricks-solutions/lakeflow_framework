...
spec. A nodespec spec is a graph of three node kinds:
...
Casing: nodespec input is snake_case; the emitted flow spec is the backend's
...
def _to_backend_keys(obj: Any) -> Any:
    ...
    if isinstance(obj, dict):
        return {_NODESPEC_TO_BACKEND_KEYS.get(k, k): _to_backend_keys(v) for k, v in obj.items()}
...
class NodespecSpecTransformer(BaseSpecTransformer):
    ...
    # Nodespec warns about inline source transformations at the node level (where
    ...
    def _process_spec(self, spec_data: Dict) -> Union[Dict, List[Dict]]:
        """Transform a nodespec spec into one flow spec, or a list when it
