...
def _format_mirror_message(message: str, kwargs: Dict[str, Any]) -> str:
    if not kwargs:
        return message
...
class CompositeLogger:
    ...
    def _call_mirror(self, method_name: str, message: str, args: tuple, kwargs: Dict[str, Any]) -> None:
        ...
        if kwargs:
            method(_format_mirror_message(message, kwargs))
        elif args:
            method(message, *args)
        else:
            method(message)
...
def resolve_logger(
    ...
    if effective_config.get("mirror_to_stdout", True):
