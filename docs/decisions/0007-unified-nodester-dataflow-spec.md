# ADR-0007: A unified, node-based dataflow spec (`nodester`)
...
Introduce a single, unified dataflow spec, called **`nodester`**, and make it the
way pipelines are described going forward.

A `nodester` spec is a **graph of nodes**. There are exactly three node types, and
...
Because output type is just a property of a target node, a single `nodester` spec
...
The same pipeline as a `nodester` graph is a flat list of nodes that reads
...
`nodester` does not change the engine. The transformer lowers a node graph into
...
- **Inline SQL/Python sources are discouraged (warning, not breaking).** Folding
  ...
  both the legacy and `nodester` formats. The recommended replacement is a plain
...
- **Smooth adoption.** Legacy formats keep working. A migration script converts
  existing specs into `nodester`. The inline-source-transformation warning gives
...
The decisions below define how the `nodester` model behaves. They are the design
