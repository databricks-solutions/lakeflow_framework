...
The **Framework Bundle must already be deployed** in the target workspace — central/platform path or your own dev deploy. See :doc:`deploy_before_you_deploy` for deploy order.
...
If you deployed the framework yourself for local dev, the default path is under your user ``.bundle/.../files/src``. See :doc:`deploy_local_framework`.

Step 4 — Validate the bundle
============================

From **your pipeline bundle** directory (after :doc:`build_pipeline_bundle_steps`):
...
- :doc:`build_pipeline_bundle_steps` — build a pipeline bundle before deploy
- :doc:`deploy_local_framework` — deploy the framework first
- :doc:`feature_versioning_framework` — version pinning and rollback
