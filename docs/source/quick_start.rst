...
.. tip::
   ...
   **New to Asset Bundles?** The framework ships as a Declarative Automation Bundle (DAB). Authenticate the CLI first, then optionally set the workspace host in ``databricks.yml`` (or leave it unset to use the host from your CLI profile). See :doc:`deploy` for the full deploy workflow.
...
.. warning::
   ...
   Always set a unique logical environment suffix (initials, story ID, or project name) so your sample schemas and jobs do not overwrite another user's deployment in a shared workspace.
