...
For deployment modes (flat DAB deploy, wheel, wheel + overlay), see :doc:`deploy_framework_options`.
...
See :doc:`deploy_ci_cd` and :doc:`feature_versioning_framework` for version pinning in higher environments.
...
- :doc:`deploy_framework_options` — flat DAB deploy vs wheel install
- :doc:`deploy_wheel` — install ``lakeflow-framework`` as a Python wheel
- :doc:`deploy_local_pipeline_bundle` — deploy a pipeline bundle after the framework is in place
