...
2. Ensure the correct Databricks workspace is set as the workspace host field in the databricks.yml file or ensure no host is set to use the default host configured on the profile used by the Databricks CLI (Databricks CLI should be configured with credentials to access this workspace).
...
3. Run the following command from the root directory to validate the Lakeflow Framework bundle:
...
.. Note::
   Databricks CLI will deploy the bundle to the default target workspace (usually dev by default) specified in the databricks.yml file. If you want to deploy the bundle to a different target, you can specify the target host using the ``-t`` option in the deploy command.
