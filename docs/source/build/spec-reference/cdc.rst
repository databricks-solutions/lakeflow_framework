...
.. list-table::
   ...
   * - **sequence_by**
     - str
     - The column name specifying the logical order of CDC events in the source data. SDP uses this sequencing to handle change events that arrive out of order.
   * - **scd_type**
     - ``string``
...
See ``samples/feature_samples/src/dataflows/feature_samples/dataflowspec/historical_snapshot_files_datetime_recursive_and_partitioned_regex_main.json`` for a complete working example.
