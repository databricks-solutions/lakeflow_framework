...
Alphabetical index of all feature pages. Prefer the :doc:`Features hub <features>`
when browsing by category; use this list when you already know the name.

* :doc:`Auto Complete / IntelliSense <feature_auto_complete>` — VS Code IntelliSense for data flow specs
* :doc:`Builder Parallelization <feature_builder_parallelization>` — parallelize pipeline bundle builds
* :doc:`Change Data Capture (CDC) <feature_cdc>` — apply CDC merges via the data flow spec
* :doc:`Change Data Feed (CDF) <feature_cdf>` — consume Delta change data feed
...
* :doc:`Logical Environments <feature_logical_environment>` — logical env at deploy time
* :doc:`Mandatory Table Properties <feature_mandatory_table_properties>` — required Delta table properties
* :doc:`Materialized Views <feature_materialized_views>` — materialized views as targets
* :doc:`Multi-Source Streaming <feature_multi_source_streaming>` — multiple flows into one target
* :doc:`Operational Metadata <feature_operational_metadata>` — operational columns and metadata
* :doc:`Python Code, Libraries & Init Scripts <feature_python_extensions>` — ship Python with the bundle
...
* :doc:`SQL Source <feature_sql_source>` — SQL as a source type
* :doc:`Substitutions <feature_substitutions>` — token substitution in specs
...
* :doc:`Versioning - DataFlow Specs <feature_versioning_dataflow_spec>` — version and map data flow specs
* :doc:`Versioning - Framework <feature_versioning_framework>` — framework versioning and upgrades
