...
- `make html-multiversion`
  - Builds versioned docs without `sphinx-multiversion` using `build_versioned_docs.sh`.
...
- `docs/build_versioned_docs.sh` (entrypoint)
- `docs/build_versioned_docs.py` (orchestrator)
- `docs/select_versions.py` (version selection rules)
...
`select_versions.py` applies these rules:
...
- `build_versioned_docs.py` writes `versions.json` with `name`, `url`, and `is_latest`.
...
3. Run `bash docs/build_versioned_docs.sh`.
